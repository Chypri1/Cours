...
int  jeu(int civilization,SDL_Window* window, SDL_Renderer* renderer,SDL_Texture *texture,int liste_cases[N][N]){

  SDL_Texture *texture_plateau = texture;
  SDL_Texture* nouveau_plateau;
  civilization_t *civ;
  ...
  crea_plat(cases); // création de la taille des cases
  ...
  SDL_Rect rectangle = cases[4][4];
  char * image_ville="./image/ville1.png";
  affiche_ville(cases,ville,renderer,texture);
  int tour=1;
  int choix = 0;
  int reset=0;
  ...
  while(program_launched){
    while (SDL_PollEvent(&event))
      {
       texture_plateau = nouveau_plateau;
      switch (event.type)
      {
        case SDL_QUIT:
        ...
        case SDL_MOUSEBUTTONDOWN:
          ...
          for(int i =0;i< N;i++){ /* changer le N en une fonction qui retourne le max des valeurs */
            ...
            if(numx==ville[i]->x && numy==ville[i]->y && ville[i]!= NULL){
                reset = 0 ;
                int choix=fonction_bandeau_ville(cases,renderer,ville,texture);
                SDL_RenderPresent(renderer);
                if(choix == 1){
                  colon[nb_colon] = crea_colon(ville[0]->x+1,ville[0]->y);
                  affiche_colon(cases,renderer,colon,texture_plateau);
                  nouveau_plateau = texture_plateau;
                  SDL_RenderPresent(renderer);
                  texture= texture_plateau;
                  nb_colon++;
                  reset = 1;
                }
                else if(choix == 2)
            ...
            if(colon[i] != NULL){
                ...
                reset = 0 ;
                choix=fonction_bandeau_colon(cases,renderer,colon[i],liste_cases,texture_plateau);
                if(choix == 1){
                  ville[nb_ville] = crea_ville(colon[i]->x,colon[i]->y);
                  affiche_ville(cases,ville,renderer,texture_plateau);
                  nouveau_plateau = texture_plateau;
                  SDL_RenderPresent(renderer);
                  colon[nb_colon]=NULL;
                  nb_ville++;
            ...
            if(batisseur[i] != NULL){
              if(numx==batisseur[i]->x && numy==batisseur[i]->y){
                choix = 0 ;
                int choix=fonction_bandeau_batisseur(cases,renderer,batisseur[i],liste_cases, texture);
                SDL_RenderPresent(renderer);
                if(choix==1){
            ...
            if(guerrier[i] != NULL){
              if(numx==guerrier[i]->x && numy==guerrier[i]->y){
                reset = 0 ;
                choix=fonction_bandeau_guerrier(cases,renderer,guerrier[i],liste_cases,texture);
